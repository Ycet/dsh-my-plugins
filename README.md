...
3. **搜索**：在顶部输入框按名称或介绍过滤插件；
4. **启停**：点击卡片上的「禁用插件 / 启用插件」，按提示重启 `dsh web` 后生效；
5. **更新**（GitHub 来源）：点击「检查更新」查看上游是否有新版本；有更新时点击「立即更新」，完成后重启 `dsh web` 生效；
6. **移除**：点击「移除插件」，在二次确认弹窗中点击「确认移除」，重启 `dsh web` 后完成卸载。
