...
/**
 * 读取用户 Agent preset 为本地相对模块声明的展示元数据。
 * 元数据用条目 id 与模块路径共同定位，避免不同 preset 的同名条目互相覆盖。
 * @param dshHome - DSH 用户目录。
 * @returns 以 `${id}\0${module}` 为键的展示元数据。
 */
export function readAgentPresetPluginMetadata(dshHome) {
	const result = new Map();
	const presetsDir = join(dshHome, ".agent-presets");
	let presets = [];
	try {
		presets = readdirSync(presetsDir, { withFileTypes: true }).filter((entry) => entry.isDirectory());
	} catch { /* 用户 preset 目录不存在 */ }

	for (const preset of presets) {
		try {
			const raw = readFileSync(join(presetsDir, preset.name, "plugin-metadata.json"), "utf8");
			const document = JSON.parse(raw);
			if (!document || !Array.isArray(document.plugins)) continue;
			for (const plugin of document.plugins) {
				if (!plugin || typeof plugin.id !== "string" || typeof plugin.module !== "string") continue;
				if (typeof plugin.name !== "string" || plugin.name.length === 0) continue;
				result.set(`${plugin.id}\0${plugin.module}`, {
					name: plugin.name,
					description: typeof plugin.description === "string" ? plugin.description : ""
				});
			}
		} catch { /* preset 未声明元数据或文件无效 */ }
	}
	return result;
}
...
export async function computeUserPlugins(loader, dshHome) {
	...
	// preset 本地模块优先使用显式元数据；普通包读取 package.json description。
	const presetMetadata = readAgentPresetPluginMetadata(dshHome);
	for (const e of user) {
		const metadata = presetMetadata.get(`${e.entryId}\0${e.moduleName}`);
		if (metadata) {
			e.moduleName = metadata.name;
